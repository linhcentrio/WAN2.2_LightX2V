...
def download_lora_dynamic(lora_url: str, civitai_token: str = None) -> str:
    ...
    try:
        ...
        logger.error(f"Traceback: {traceback.format_exc()}")
        return None
def validate_tensor_shape(tensor, expected_shape, step_name=""):
    """Validate and log tensor shape, raise error if mismatch."""
    current_shape = list(tensor.shape)
    if current_shape != expected_shape:
        logger.error(f"❌ Shape mismatch at {step_name}: Expected {expected_shape}, got {current_shape}")
        raise ValueError(f"Tensor shape mismatch at {step_name}: {current_shape} vs {expected_shape}")
    logger.info(f"✅ Valid shape at {step_name}: {current_shape}")
    return tensor
...
def process_image_with_aspect_control(
    ...
    """
    ENHANCED: Process image with different aspect ratio modes, full validation for multi-size support.
    - Auto-detect orientation for auto_720p.
    - Validate tensor shape at each step to prevent mismatches.
    - Support flexible sizes (256-1536px), log debug info.
    """
    try:
        # Initialize nodes
        ...
        # Validate initial shape: Expect [1, H, W, 3] or [H, W, 3]
        if loaded_image.ndim == 3:
            loaded_image = loaded_image.unsqueeze(0)  # Add batch dim if missing
        validate_tensor_shape(loaded_image, [1, loaded_image.shape[1], loaded_image.shape[2], 3], "Initial load")
        # Get original dimensions
        _, orig_height, orig_width, _ = loaded_image.shape
        logger.info(f"🖼️ Original image: {orig_width}x{orig_height}")
        # Auto 720p detection and adjustment
        if auto_720p:
        ...
        # Mode-specific processing with validation
        if aspect_mode == "preserve":
            ...
            validate_tensor_shape(scaled_image, [1, new_height, new_width, 3], "Post-scale (preserve)")
            # Padding if needed
            if new_width != target_width or new_height != target_height:
                pad_x = (target_width - new_width) // 2
                pad_y = (target_height - new_height) // 2
                # Permute to BCHW for padding
                padded = F.pad(
                ...
                final_image = padded.permute(0, 2, 3, 1)  # BCHW -> BHWC
            else:
                final_image = scaled_image
            validate_tensor_shape(final_image, [1, target_height, target_width, 3], "Post-pad (preserve)")
            ...
            validate_tensor_shape(cropped, [1, crop_height, crop_width, 3], "Post-crop")
            # Scale to target
            final_image = image_scaler.upscale(
                cropped, "lanczos", target_width, target_height, "disabled"
            )[0]
            validate_tensor_shape(final_image, [1, target_height, target_width, 3], "Post-scale (crop)")
            logger.info(f"✂️ Crop mode: {orig_width}x{orig_height} -> crop({crop_width}x{crop_height}) -> {target_width}x{target_height}")
        else:  # stretch
            # Stretch: Direct scale
            final_image = image_scaler.upscale(
                loaded_image, "lanczos", target_width, target_height, "disabled"
            )[0]
            validate_tensor_shape(final_image, [1, target_height, target_width, 3], "Post-scale (stretch)")
            logger.info(f"🔄 Stretch mode: {orig_width}x{orig_height} -> {target_width}x{target_height}")
        # Final RGB check (ensure 3 channels)
        if final_image.shape[-1] != 3:
            logger.warning(f"⚠️ Channel mismatch: {final_image.shape[-1]} -> Forcing to RGB")
...
def save_video_optimized(frames_tensor, output_path, fps=16):
    ...
    try:
        ...
        file_size_mb = os.path.getsize(output_path) / (1024 * 1024)...  # SIMPLIFIED - just log stats, no validation
...
def validate_input_parameters(job_input: dict) -> tuple[bool, str]:
    ...
    try:
        ...
        if not (256 <= width <= 1536 and 256 <= height <= 1536):  # Updated limit for multi-size support
